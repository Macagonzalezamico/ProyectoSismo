...
class InfoExtractorJaponURL(iex.InfoExtractor):
    ...
    def extractInfo(self, country: str, source: str, fromDateTime: datetime, toDateTime: datetime ):
        ...
        for fecha in lista_fechas:
            incremental = dt.datetime.strptime(fecha, "%Y/%m/%d")
            lista_fechas_incremental.append(incremental)

        #INDICES Fechas
        indices = []
        for indice, elemento in enumerate(lista_fechas_incremental):
            if (fromDateTime == lista_fechas_incremental[indice]) or (toDateTime == lista_fechas_incremental[indice]):
